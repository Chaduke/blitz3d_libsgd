...
If test_ferris_wheel Then
	...
	PlaceEntityOnTerrain fw\base_pivot,ta\bs\trn,22.5
	AimEntityAtEntity ta\bs\pivot,fw\wheel_pivot,0
	bm.BubbleMaker = CreateBubbleMaker()
	bm2.BubbleMaker = CreateBubbleMaker()
	bm3.BubbleMaker = CreateBubbleMaker()
	
	PlaceEntityOnTerrain bm3\model,ta\bs\trn,0.1,False,True 
...
While ta\loop 
	BeginFrame ta
	UnrealMouseInput ta\bs\pivot
	KeyboardInputFirstPerson ta\bs\pivot
	KeepEntityAboveTerrain ta\bs\pivot,ta\bs\trn
	...
	If test_ferris_wheel Then 
		If (IsKeyHit(KEY_SPACE)) Then 
			If boarded = True Then 
				boarded = False 
				RemovePassenger fw,ta\bs\pivot,ta\bs\trn 
			Else 
				boarded = True 
				BoardPassenger fw,ta\bs\pivot,Rand(0,15)
			End If
		End If 	 			
