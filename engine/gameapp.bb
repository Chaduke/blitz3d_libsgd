...
Function CreateGameApp.GameApp(title$="Game App",subtitle$="Programmer's",fullscreen=1)
	...
	g\game_state = GAME_STATE_START_MENU
	g\npcs = True 
	g\loop = True 
	g\quit = False 
...
Function BeginFrame(g.GameApp)	
	...
	If IsKeyHit(KEY_ESCAPE) Then 
		If g\quit = False Then g\quit = True Else g\quit = False
	End If 			
	If (IsKeyHit(KEY_Y) And g\quit=True) Then g\loop = False				
End Function

Function EndFrame(g.GameApp)			
	RenderScene()
	Clear2D()	
	If g\quit Then 
		DrawTitle g
