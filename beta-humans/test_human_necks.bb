; test_human_necks.bb
; by Chaduke
; 20240809

Include "../engine/gameapp.bb"
Include "../engine/gui.bb"
Include "../engine/environment.bb"
Include "human_neck.bb"

ga.GameApp = CreateGameApp("Human Neck Tester","Chaduke's",1,False)
MoveEntity ga\pivot,0,1,0
e.Environment = CreateEnvironment(ga\gui_font)
ground = CreateGround()

For i = 0 To 3
	hn.HumanNeck = CreateHumanNeck()
	MoveEntity hn\model,-1.5 + i,1,3
Next 	

ga\info$ = "A fine selection of wonderful human necks for your viewing pleasure master..."

While ga\loop
	BeginFrame ga
	If IsKeyHit(KEY_F1) Then ShowHideGUIWindow e\gui
	For hn.HumanNeck = Each HumanNeck
		UpdateHumanNeck hn
	Next
	If (GUIDraggedCheck() = False And WidgetDraggedCheck()=False) Then 	SimpleNavigation ga
	UpdateEnvironment e,GUIDragCheck()	: DrawAllGUIs()
	EndFrame ga
Wend 
End
